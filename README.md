...
- 在Account组件中添加了一个切换主题的按钮，点击按钮之后会切换白天和黑夜模式，主要是通过按钮给全局添加一个.dark的class，这个class会切换body的class，body的class会切换对应的css样式，这样就可以切换主题了。所有的字体都变成白色，背景是黑色的，一些输入框和方框则是灰色，有一点高亮，视觉上更好看一点。

- 在Discover中的Waterfall中添加了下拉刷新的功能，主要是使用React-Vant里面的PullRefresh 组件实现，当我下拉刷新的时候会调用useBookStore.js里面的refreshBooks函数进行刷新数据。


- 调用了Coze工作流完成了书籍的自动编写功能，提供书籍的标题和简介就可以通过Coze工作流自动编写书籍的内容，提供了书籍的自动编写功能。
...
- 给AI助手添加了一个流式输出的功能，当我发送消息的时候，会立即显示在页面上，而不是等AI助手回复完了才显示，这样可以提高用户的体验。
